...
import { ref } from 'vue'
import DungeonTabsPrototype from '@/ui/prototypes/dungeon/DungeonTabsPrototype.vue'

const meta: Meta<typeof DungeonTabsPrototype> = {
  title: 'Prototypes/Dungeon/DungeonTabsPrototype',
  component: DungeonTabsPrototype,
  tags: ['autodocs'],
  parameters: {
    layout: 'padded',
  },
...
export default meta
type Story = StoryObj<typeof DungeonTabsPrototype>
 ...
 *   → L-bracket mounts at each ring corner (16px/3px/6px, same as DungeonFramePrototype)
 ...
 * via a local ref within the render function — Storybook args drive initial
 * state but the component is fully interactive within the story.
 */
export const Default: Story = {
  render: (args) => ({
    components: { DungeonTabsPrototype },
    setup() {
      const active = ref(args.modelValue ?? 0)
      return { args, active }
    },
    template: `<DungeonTabsPrototype :tabs="args.tabs" :variant="args.variant" v-model="active" />`,
  }),
}
    ...
    template: `
      <div style="display: flex; flex-direction: column; gap: 24px; align-items: flex-start;">
        <div>
          ...
          <DungeonTabsPrototype :tabs="['Overview', 'Risks', 'History']" variant="default" v-model="a0" />
        </div>
        <div>
          <p style="margin: 0 0 8px; font-family: monospace; font-size: 11px; color: #7a6c44;">variant="aged"</p>
          <DungeonTabsPrototype :tabs="['Overview', 'Risks', 'History']" variant="aged" v-model="a1" />
        </div>
        <div>
          <p style="margin: 0 0 8px; font-family: monospace; font-size: 11px; color: #7a6c44;">variant="accent"</p>
          <DungeonTabsPrototype :tabs="['Overview', 'Risks', 'History']" variant="accent" v-model="a2" />
        </div>
      </div>
    `,
  }),
 ...
 *   · 8px chamfer at each corner — identical to DungeonFramePrototype
  ...
  render: () => ({
    ...
    template: `
      <div style="background: #0d1018; padding: 48px 48px 56px; border-radius: 4px; display: flex; flex-direction: column; gap: 32px; align-items: flex-start;">
        <DungeonTabsPrototype :tabs="['Architecture', 'Risks', 'History']" variant="default" v-model="a0" />
        <DungeonTabsPrototype :tabs="['Architecture', 'Risks', 'History']" variant="accent"  v-model="a1" />
      </div>
    `,
  }),
  parameters: {
