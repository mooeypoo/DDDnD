<template>
  <!--
    DungeonTabsPrototype — horizontal segmented tab strip in the dungeon-console family.

    Structural layers (outer → inner):

      1. dungeon-tabs (outer shell div)
                              Same clip-path octagon + dark shell + ambient
                              drop-shadow as DungeonFramePrototype. Uses --dng-chamfer
      ...
      2. dungeon-tabs__ring   Cast-bronze ring — identical gradient + directional
                              ...
                              geometry as DungeonFramePrototype.
      ...
      3. dungeon-tabs__strip  Horizontal flex row. Spans the ring interior full-width
                              ...
                              DungeonFramePrototype nameplate — bronze visible only
      ...
      4. dungeon-tab-item     Each clickable tab zone. Fills the strip's full height.
                              ...
                              Inactive state:
                                · --dng-panel-surface background — same recessed teal-dark
                                  as the main inset on DungeonFramePrototype. Reads as
                                ...
                                · 1px --dng-divider inter-tab separator on right edge
                                  (::before pseudo-element to avoid :last-child special-casing).

                              Active state (.is-active):
                                · Same plate gradient (bg-hi → bg-mid → bg-base) as the
                                  DungeonButtonPrototype face plate and the DungeonFramePrototype
    ...
    Structural relationship to the family:
      · Outer shell clip-path and drop-shadow are identical to DungeonFramePrototype.
      ...
      · Active tab face reuses the plate gradient + bevel border strategy from
        DungeonButtonPrototype's face plate — no new structural invention.
      ...
      · The full-bleed strip technique (negative margin on the strip) mirrors the
        DungeonFramePrototype nameplate strategy exactly.
...
/* ─────────────────────────────────────────────────────────────
   ...
   Same strategy as DungeonFramePrototype.variant-aged:
...
/* ─────────────────────────────────────────────────────────────
   ...
   (same layer as the button primary variant and the frame accent
...
/* ─────────────────────────────────────────────────────────────
   ...
   to the ring's inner boundary edges, mirroring the DungeonFramePrototype
   nameplate bleed strategy exactly. Hard border lines at left/right
   edges close the structural gap at the ring boundary (these sit
   between ring padding and the strip, so only the inner ring edge
   is visible — not an additional outer line).
...
/* ─────────────────────────────────────────────────────────────
   ...
   Inactive (default):
     · --dng-panel-surface background — same recessed teal-dark used
       as DungeonFramePrototype's content inset surface. Reads as
       set below the ring plane, not projecting outward.
   ...
   Active (.is-active):
     · Full plate gradient (bg-hi → bg-mid → bg-base) — same as
       DungeonButtonPrototype face plate. Raised-plate appearance
       comes from the lighter background + bright directional bevel
       alone — no physical height change.
